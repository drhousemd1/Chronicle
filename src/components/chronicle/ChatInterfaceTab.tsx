    ...
    <div className={`flex flex-1 h-full w-full overflow-hidden relative ${darkMode ? 'bg-slate-900' : 'bg-black'}`}>
      ...
      <aside className={`w-[300px] flex-shrink-0 border-r border-slate-200 flex flex-col h-full shadow-[inset_-4px_0_12px_rgba(0,0,0,0.02)] z-10 transition-colors relative overflow-hidden ${showBackground ? 'bg-white/90 backdrop-blur-md' : 'bg-white'}`}>
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <section
            className="flex-shrink-0 rounded-xl p-4 border border-slate-200 shadow-lg transition-all duration-700 relative overflow-hidden* Preloaded background images — all mounted, only active one visible */}
            {(['sunrise', 'day', 'sunset', 'night'] as const).map((time) => (
              <img
                key={time}
                src={getTimeBackgroundImage(time)}
                alt=""
                className={`absolute inset-0 w-full h-fullobject-center  object-cover nsition-opacity duration-700 ${
