...
export const AvatarGenerationModal: React.FC<AvatarGenerationModalProps> = ({
  ...
  // Build suggested prompt from character data
  const generateSuggestedPrompt = (): string => {
    const parts: string[] = [];
    
    if (characterData?.sexType) {
      parts.push(characterData.sexType);
    }
    if (characterData?.age) {
      parts.push(`${characterData.age} years old`);
    }
    ...
    if (wearing) {
      ...
      if (clothingParts.length > 0) {
        parts.push(`wearing ${clothingParts.join(" and ")}`);
      }
    }
