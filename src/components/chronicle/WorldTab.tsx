...
import { Sunrise, Sun, Sunset, Moon, ChevronUp, ChevronDown, Pencil, Sparkles, Share2, Trash2, Plus, X, Info, Lock, BrainCog } from 'lucide-react';
    ...
    <div className="flex flex-1 h-full overflow-hidden">
      <aside className="w-[300px] flex-shrink-0 bg-[#2a2a2f] shadow-[inset_-1px_0_0_rgba(255,255,255,0.06)] flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-none pb-20 bg-[#2a2a2f]">
          ...
          <section className="space-y-2">
            <button type="button" aria-expanded={!sideCharsCollapsed} className="relative w-full overflow-hidden bg-gradient-to-b from-[#5a7292] to-[#4a5f7f] border-t border-white/20 px-5 py-2 rounded-xl mb-3 shadow-lg cursor-pointer select-none text-left" onClick={() => setSideCharsCollapsed(prev => !prev)}>
              <div className="absolute inset-0 z-0 bg-gradient-to-tr from-white/10 to-transparent opacity-40" style={{ height: '60%' }} />
              <div className="flex items-center justify-between relative z-[1]">
                <span className="text-[10px] font-bold text-white uppercase tracking-wider">Side Characters</span>
