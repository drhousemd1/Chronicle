...
function selectSupportingExcerpts(
  ...
  for (const i of bridgeIndices) picked.set(i, olderPool[i]);
  for (const s of ranked) {
    if (picked.size >= supportingLimit) break;
    picked.set(s.idx, s.msg);
  }
  // Return in original chronological order
  return [...picked.entries()].sort((a, b) => a[0] - b[0]).map(([, m]) => m);
}
...
// ============================================================================
function streamTextAsSSE(text: string, modelId: string, headers: HeadersInit): Response {
...
function jsonResponseAsCompletion(text: string, modelId: string, headers: HeadersInit): Response {
  const payload = {
...
async function runRoleplayV2(
  ...
  maxTokens: number,
  ctx: RoleplayContext | undefined,
): Promise<{ ok: true; finalText: string } | { ok: false; reason: string }> {
  // ---- 1) Planner ---------------------------------------------------------
  const lastUser = [...messages].reverse().find((m) => m.role === 'user')?.content ?? '';
  const recent = messages.slice(-RECENT_HISTORY_WINDOW);
  const supporting = selectSupportingExcerpts(messages, RECENT_HISTORY_WINDOW, SUPPORTING_HISTORY_WINDOW, ctx);
...
- Supporting older excerpts may inform facts but must NEVER override the latest user turn.`,
  ...
  if (!writerCall.ok) return { ok: false, reason: `writer_status_${writerCall.status}` };
  let writerDraft = '';
  try {
    const data = await writerCall.response.json();
    writerDraft = data?.choices?.[0]?.message?.content ?? '';
  } catch {
    return { ok: false, reason: 'writer_parse_failed' };
  }
  if (!writerDraft.trim()) return { ok: false, reason: 'writer_empty' };
...
${JSON.stringify(plan)}`,
  ...
  if (!finalText.trim()) return { ok: false, reason: 'empty_after_normalize' };
...
async function handleDirect(
  ...
  if (result.status === 403) {
    ...
    if (retry.ok) {
      if (stream) {
        return new Response(retry.response.body, {
          headers: {
            ...responseHeadersBase,
            "Content-Type": "text/event-stream",
            "Cache-Control": "no-cache",
...
serve(async (req) => {
  ...
  try {
    ...
    const { messages, stream = true, max_tokens: maxTokens = 4096, pipeline = 'direct', roleplayContext } = body;
    ...
    return await handleDirect(messages, modelId, stream, maxTokens, responseHeadersBase);
  } catch (error) {
